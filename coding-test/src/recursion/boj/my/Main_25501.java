...
public class Main_25501 {
    ...
    구체적으로는, 문자열S를 isPalindrome 함수의 인자로 전달하여 팰린드롬 여부를 반환값으로 알아낼 것이다.
    더불어 판별하는 과정에서 recursion 함수를 몇 번 호출하는지 셀 것이다.
    정휘를 따라 여러분도 함수의 반환값과 recursion 함수의 호출 횟수를 구해보자.

    입력
    첫째줄에 테스트케이스의 개수(1<= T <= 1000)
    둘째줄부터 T개의 줄에 알파벳 대문자로 구성된 문자열 S

    출력
    각 테스트케이스마다, isPalindrome 함수의 반환값과 recursion 함수의 호춧 횟수를 한 줄에 공백으로 구분하여 출력
     */
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < t; i++) {
            ...
            count = 0;
            int palindromeResult = isPalindrome(word);
            sb.append(palindromeResult).append(" ").append(count).append('\n');
        }
