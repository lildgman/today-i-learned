...
public class Main_1620 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // 포켓몬 정보를 저장
        // 번호 -> 이름
        String[] pokeByNum = new String[n + 1];
        
        // 이름 -> 번호
        HashMap<String, Integer> pokeByName = new HashMap<>();

        // 포켓몬 입력
        for (int i = 1; i <= n; i++) {
            String name = br.readLine();
            pokeByNum[i] = name;
            pokeByName.put(name, i);
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (Character.isDigit(search.charAt(0))) { // 숫자가 들어왔으면
                int num = Integer.parseInt(search);
                sb.append(pokeByNum[num]);
            } else {
                sb.append(pokeByName.get(search));
            }
            sb.append('\n');
